...
streamlit.text('🥗Kale, Spinach & Mocket Smoothie')
...
try:
  ...
  if not fruit_choice:
    ...
    #streamlit.text(fruityvice_response.json()) - json (removed)
    #fruityvice_normalized = pandas.json_normalize(fruityvice_response.json())
...
if fruit_choice1 = streamlit.text_input('What fruit would you like to add?')   
...
streamlit.stop()
#the ff codes are the old one without using function
#import snowflake.connector
my_cnx = snowflake.connector.connect(**streamlit.secrets["snowflake"])
